...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    ((b.up, Nove.Up :: history) :: (b.down, Move.Down :: history) ::
      (b.left, Move.Left :: history) :: (b.right, Move.Right :: history))
      .filter(a => a._1.b1.isLegal && a._1.b2.isLegal)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.flatMap(a => neighborsWithHistory(a._1, a._2)).filter(a => explored.toList.indexOf(a) == -1)
  ...
  lazy val solution: List[Move] = {
    ...
    val a =
      (for
      {(block, moves) <- pathsToGoal}
        yield moves.length).min
    pathsToGoal.toList.filter(b => b._2.length == a)
  }